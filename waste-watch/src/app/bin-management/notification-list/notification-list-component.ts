...
export class BinNotificationListComponent implements OnInit {
  faBell = faBell;
  faFilter = faFilter;
  faCheck = faCheck;
  faTimes = faTimes;
  ...
  loadNotifications() {
    ...
    this.binService.getBinNotifications().pipe(
      catchError(error => {
  ...
  async clearAllNotifications() {
    ...
    try {
      for (const id of notificationIds) {
        try {
          await this.binService.deleteBinNotification(id).toPromise();
